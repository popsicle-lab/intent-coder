...
> **Supersedes**: ——
> **Related PDRs**: `PDR-0001`（auth 策略，acceptance.AdminRevokeTargetSession 依赖即时撤销）
> **Related RFC**: `session-storage.rfc.md`
> **Related Journey**: `J-0001`（新员工入职当天开通所有权限，Stage 3 鉴权）
...
billing / admin-console 每次鉴权都依赖 auth 校验 session，但 session 的存储形态一直
`[Awaiting ADR-0003]`，导致 contracts.intent 的「会话校验」契约无法收紧。需定形以解锁。

### 多角色辩论摘要

继承 `session-storage.arch-debate.md`。
...
| 方案 | 提案者 | 否决理由 |
|------|--------|---------|
| JWT 无状态 | PERF | 撤销需黑名单，退回有状态，违背即时撤销 NFR |
| DB session 表 | DEV(代述) | 热路径打 DB，延迟最差 |

详见 `session-storage.tech-decision-matrix.md`（A 加权 4.55 领先）。
...
|-----------|---------|----------|------|
| `intents/contracts.intent` | 解锁 | goal "auth 对外提供可信的会话校验" | 解锁后登记 [ADR-0003 Accepted] |
| `intents/session.intent` | 新增 | safety `SessionResolvesToOwner` + intent `VerifySession` | intent-spec-writer 收紧 |
| `docs/invariants/*.intent` (全局) | 无影响 | —— | 不触及全局，无需 CADR |

> 不触及 charter 四铁律 / Layer Map → 普通 ADR 即可，无需 CADR。
...
- 跑 `intent check products/auth/intents/session.intent`：`SessionResolvesToOwner`
  verified，`VerifySession` 不破坏隔离不变量，与既有 auth intent 无矛盾。
...
- **Contracts Seed**: `session-storage.contracts.intent`
